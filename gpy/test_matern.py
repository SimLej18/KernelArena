"""Matern 5/2 Kernel benchmarks — GPy."""
import numpy as np
...
def test_1d_regular(benchmark, request):
    ...
    def setup():
        ...
        kernel = GPy.kern.Matern52(input_dim=1, variance=1.0, lengthscale=ls)
        return (kernel, x, x), {}
...
def test_2d_regular(benchmark, request):
    ...
    def setup():
        ls = float(next(ls_iter))
        kernel = GPy.kern.Matern52(input_dim=2, variance=1.0, lengthscale=ls)
        return (kernel, x, x), {}
