"""Matern 5/2 Kernel benchmarks — GPJax."""
import numpy as np
...
def test_1d_regular(benchmark, request):
	...
	def setup():
		...
		kernel = Matern52(lengthscale=jnp.array([ls]))
		kernel.cross_covariance(x, x).block_until_ready()
		return (kernel, x, x), {}
...
def test_1d_random(benchmark, request):
	...
	def setup():
		...
		ls = float(next(ls_iter))
		kernel = Matern52(lengthscale=jnp.array([ls]))
		kernel.cross_covariance(x_setup, x_setup).block_until_ready()
		return (kernel, x, x), {}
...
def test_2d_regular(benchmark, request):
	...
	def setup():
		ls = float(next(ls_iter))
		kernel = Matern52(lengthscale=jnp.array([ls, ls]))
		kernel.cross_covariance(x, x).block_until_ready()
		return (kernel, x, x), {}
